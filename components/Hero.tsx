...
const Hero: React.FC = () => {
  ...
  return (
    <section id="trang-chu" className="relative h-[500px] md:h-[650px] overflow-hidden group">
      ...
      {/* Info Overlay */}
      <div className="absolute top-1/2 left-4 md:left-20 -translate-y-1/2 z-10 bg-[#102E50]/95 backdrop-blur-lg p-10 md:p-14 rounded-3xl max-w-md text-white shadow-[0_20px_60px_rgba(0,0,0,0.5)] hidden md:block">
        <h2 className="font-merriweather text-4xl md:text-5xl font-black mb-6 leading-tight">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={i} className="text-center p-3 bg-white/10 rounded-2xl hover:bg-[#FCF9EA] hover:text-[#102E50] transition-all cursor-default group">
              <span className="block text-3xl font-black font-merriweather">{stat.n}</span>
              <span className="text-[9px] tracking-widest font-bold group-hover:text-[#102E50]">{stat.l}</span>
            </div>
          ))}
        </div>
      </div>
